...
private:
    ...
    // Command queues
    stl::unique_ptr<DX12CommandQueue> m_DirectQueue;
    stl::unique_ptr<DX12CommandQueue> m_ComputeQueue;
    stl::unique_ptr<DX12CommandQueue> m_CopyQueue;

    // Contexts (triple buffered for graphics)
    stl::array<stl::unique_ptr<DX12GraphicsContext>, MAX_FRAMES_IN_FLIGHT> m_GraphicsContexts;
    stl::unique_ptr<DX12ComputeContext> m_ComputeContext;
    stl::unique_ptr<DX12CopyContext> m_CopyContext;

    // Descriptor heaps
    stl::unique_ptr<DX12DescriptorHeap> m_CbvSrvUavHeap;
    stl::unique_ptr<DX12DescriptorHeap> m_RtvHeap;
    stl::unique_ptr<DX12DescriptorHeap> m_DsvHeap;
    stl::unique_ptr<DX12DescriptorHeap> m_SamplerHeap;

    // Memory allocator
    stl::unique_ptr<DX12MemoryAllocator> m_MemoryAllocator;

    // Pipeline states (owned by device)
    stl::vector<stl::unique_ptr<DX12PipelineState>> m_PipelineStates;
