...
FixedBlockPool::FixedBlockPool(LinearArena *arena, size_t block_size, size_t blocks_per_chunk)
	: arena_(arena), blk_size_(std::max(static_cast<size_t>(sf::align_up(block_size, alignof(std::max_align_t))), sizeof(Node))), per_chunk_(blocks_per_chunk), free_(nullptr),
	  chunks_allocated_(0) {
