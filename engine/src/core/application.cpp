...
namespace sf {
    ...
    Application::Application(const ApplicationCreationDesc& desc) : m_Name(mem::MemTag::Strings, desc.name), m_Minimized(false) {
        PROFILE_FUNCTION();
        ...
        m_Window = stl::make_unique<Window>(
            mem::MemTag::Logic,
            WindowParams{desc.width, desc.height, stl::string(mem::MemTag::Strings, desc.name), BIND_EVENT_FN(Application::on_event)});
    }
