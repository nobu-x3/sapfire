...
namespace sf {
    Window::Window(const WindowParams& params) :
        ...
        // Create SDL window
        m_Window =
            SDL_CreateWindow(params.name.c_str(), static_cast<int>(params.width), static_cast<int>(params.height), SDL_WINDOW_RESIZABLE);
        if (!m_Window) {
    ...
    void Window::handle_sdl_event(const SDL_Event& event) {
        ...
        case SDL_EVENT_MOUSE_MOTION:
            {
                MouseMovedEvent mouse_moved(static_cast<i32>(event.motion.x), static_cast<i32>(event.motion.y));
