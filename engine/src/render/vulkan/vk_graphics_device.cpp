...
void VkGraphicsDevice::init_command_queues() {
    ...
    m_GraphicsQueue = stl::make_unique<VkCommandQueue>(sf::mem::Engine_Rendering, m_Device, graphics_queue, CommandQueueType::Direct, "Graphics Queue");
    m_ComputeQueue = stl::make_unique<VkCommandQueue>(sf::mem::Engine_Rendering, m_Device, compute_queue, CommandQueueType::Compute, "Compute Queue");
    m_TransferQueue = stl::make_unique<VkCommandQueue>(sf::mem::Engine_Rendering, m_Device, transfer_queue, CommandQueueType::Copy, "Transfer Queue");
}
...
void VkGraphicsDevice::end_frame() {
    get_current_graphics_context().close();
}
...
Texture& VkGraphicsDevice::get_current_back_buffer() {
    return m_BackBuffers[m_CurrentBackBufferIndex];
}

Texture& VkGraphicsDevice::get_back_buffer(u32 index) {
...
IGraphicsContext& VkGraphicsDevice::get_current_graphics_context() {
    return *m_GraphicsContexts[m_CurrentFrameIndex];
}

IGraphicsContext& VkGraphicsDevice::get_graphics_context(u32 frame_index) {
    return *m_GraphicsContexts[frame_index];
}

IComputeContext& VkGraphicsDevice::get_compute_context() {
    return *m_ComputeContext;
}

ICopyContext& VkGraphicsDevice::get_copy_context() {
