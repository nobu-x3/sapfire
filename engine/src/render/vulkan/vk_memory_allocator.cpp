...
VkMemoryAllocator::VkMemoryAllocator(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device)
    : m_Device(device) {
...
void VkMemoryAllocator::free_buffer(Buffer& buffer) {
    if (buffer.resource && buffer.allocation) {
        vmaDestroyBuffer(m_Allocator, reinterpret_cast<VkBuffer>(buffer.resource),
...
void VkMemoryAllocator::free_texture(Texture& texture) {
    if (texture.resource && texture.allocation) {
        vmaDestroyImage(m_Allocator, reinterpret_cast<VkImage>(texture.resource),
