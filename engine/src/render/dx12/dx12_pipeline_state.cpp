...
namespace sf::render::dx12 {
    ...
    void DX12PipelineState::create_graphics(ID3D12Device* device, ID3D12RootSignature* root_sig, const GraphicsPipelineStateDesc& desc) {
        m_IsCompute = false;
        // Compile shaders
        auto vs_result = sf::render::dx12::compile(sf::render::dx12::ShaderType::Vertex, desc.shader_module.vertex_shader_path,
                                                   desc.shader_module.vertex_entry_point);
        auto ps_result = sf::render::dx12::compile(sf::render::dx12::ShaderType::Pixel, desc.shader_module.pixel_shader_path,
        ...
        for (size_t i = 0; i < desc.blend_states.size() && i < 8; ++i) {
            ...
            blend_desc.RenderTarget[i] = {.BlendEnable = blend.blend_enable ? TRUE : FALSE,
                                          .LogicOpEnable = FALSE,
        ...
        D3D12_RASTERIZER_DESC rasterizer_desc = {.FillMode = to_d3d12_fill_mode(desc.rasterizer.fill_mode),
        ...
        D3D12_DEPTH_STENCIL_DESC depth_stencil_desc = {
            ...
            .FrontFace = {.StencilFailOp = to_d3d12_stencil_op(desc.depth_stencil.front_face.fail_op),
            ...
            .BackFace = {.StencilFailOp = to_d3d12_stencil_op(desc.depth_stencil.back_face.fail_op),
    ...
    void DX12PipelineState::create_compute(ID3D12Device* device, ID3D12RootSignature* root_sig, const ComputePipelineStateDesc& desc) {
        ...
        auto cs_result = sf::render::dx12::compile(sf::render::dx12::ShaderType::Compute, desc.shader_path, desc.entry_point);
        D3D12_COMPUTE_PIPELINE_STATE_DESC pso_desc = {
            .pRootSignature = root_sig,
            .CS = {.pShaderBytecode = cs_result.shader_blob->GetBufferPointer(), .BytecodeLength = cs_result.shader_blob->GetBufferSize()},
            .NodeMask = 0,
            .CachedPSO = {.pCachedBlob = nullptr, .CachedBlobSizeInBytes = 0},
            .Flags = D3D12_PIPELINE_STATE_FLAG_NONE};
