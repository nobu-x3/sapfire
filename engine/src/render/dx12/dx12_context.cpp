...
namespace sf::render::dx12 {
    ...
    void DX12Context::close() { dx12_check(m_CommandList->Close()); }
    ...
    DX12GraphicsContext::DX12GraphicsContext(DX12GraphicsDevice* device) : m_Device(device) {
    ...
    void DX12GraphicsContext::set_viewport(const Viewport& viewport) {
        D3D12_VIEWPORT vp = {.TopLeftX = viewport.x,
                             .TopLeftY = viewport.y,
    ...
    void DX12GraphicsContext::set_scissor_rect(const ScissorRect& scissor) {
        D3D12_RECT rect = {.left = scissor.left, .top = scissor.top, .right = scissor.right, .bottom = scissor.bottom};
    ...
    void DX12GraphicsContext::set_index_buffer(Buffer& buffer, Format format) {
        D3D12_INDEX_BUFFER_VIEW ibv = {.BufferLocation = static_cast<ID3D12Resource*>(buffer.resource)->GetGPUVirtualAddress(),
    ...
    void DX12GraphicsContext::draw_indexed_instanced(u32 index_count_per_instance, u32 instance_count, u32 start_index, i32 base_vertex,
                                                     u32 start_instance) {
    ...
    DX12ComputeContext::DX12ComputeContext(DX12GraphicsDevice* device) : m_Device(device) {
        ID3D12Device* d3d_device = device->get_d3d12_device();
    ...
    DX12CopyContext::DX12CopyContext(DX12GraphicsDevice* device) : m_Device(device) {
        ID3D12Device* d3d_device = device->get_d3d12_device();
        dx12_check(d3d_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COPY, IID_PPV_ARGS(&m_CommandAllocator)));
        dx12_check(d3d_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COPY, m_CommandAllocator.Get(), nullptr,
                                                 IID_PPV_ARGS(&m_CommandList)));
        dx12_check(m_CommandList->Close());
    }
    ...
    void DX12CopyContext::copy_buffer_to_texture(Texture& dst, Buffer& src, u32 subresource) {
        ...
        D3D12_TEXTURE_COPY_LOCATION dst_location = {
            .pResource = dst_resource, .Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX, .SubresourceIndex = subresource};
        D3D12_TEXTURE_COPY_LOCATION src_location = {
            .pResource = src_resource, .Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT, .PlacedFootprint = layout};
