...
namespace sf::render::dx12 {
    DX12DescriptorHeap::DX12DescriptorHeap(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heap_type, u32 descriptor_count,
                                           const wchar_t* name) :
    ...
    u32 DX12DescriptorHeap::allocate_srv(Buffer& buffer) {
        ...
        D3D12_SHADER_RESOURCE_VIEW_DESC srv_desc = {.Format = DXGI_FORMAT_UNKNOWN,
    ...
    u32 DX12DescriptorHeap::allocate_srv(Texture& texture, const ShaderResourceViewDesc* desc) {
        u32 index = m_CurrentIndex++;
        D3D12_SHADER_RESOURCE_VIEW_DESC srv_desc = {.Format = desc ? to_dxgi_format(desc->format) : to_dxgi_format(texture.format),
                                                    .ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D,
                                                    .Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING,
                                                    .Texture2D = {.MostDetailedMip = desc ? desc->most_detailed_mip : 0u,
    ...
    u32 DX12DescriptorHeap::allocate_uav(Buffer& buffer) {
        u32 index = m_CurrentIndex++;
        D3D12_UNORDERED_ACCESS_VIEW_DESC uav_desc = {.Format = DXGI_FORMAT_UNKNOWN,
    ...
    u32 DX12DescriptorHeap::allocate_uav(Texture& texture, const UnorderedAccessViewDesc* desc) {
        u32 index = m_CurrentIndex++;
        D3D12_UNORDERED_ACCESS_VIEW_DESC uav_desc = {.Format = desc ? to_dxgi_format(desc->format) : to_dxgi_format(texture.format),
                                                     .ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D,
                                                     .Texture2D = {.MipSlice = desc ? desc->mip_slice : 0u, .PlaneSlice = 0}};
        D3D12_CPU_DESCRIPTOR_HANDLE handle = get_cpu_descriptor_handle(index);
    ...
    u32 DX12DescriptorHeap::allocate_cbv(Buffer& buffer) {
        u32 index = m_CurrentIndex++;
        D3D12_CONSTANT_BUFFER_VIEW_DESC cbv_desc = {.BufferLocation = static_cast<ID3D12Resource*>(buffer.resource)->GetGPUVirtualAddress(),
                                                    .SizeInBytes =
                                                        calculate_constant_buffer_byte_size(static_cast<u32>(buffer.size_in_bytes))};
        D3D12_CPU_DESCRIPTOR_HANDLE handle = get_cpu_descriptor_handle(index);
    ...
    u32 DX12DescriptorHeap::allocate_cbv(const ConstantBufferViewDesc& desc) {
        u32 index = m_CurrentIndex++;
        D3D12_CONSTANT_BUFFER_VIEW_DESC cbv_desc = {.BufferLocation = desc.buffer_location,
    ...
    u32 DX12DescriptorHeap::allocate_rtv(Texture& texture, const RenderTargetViewDesc* desc) {
        u32 index = m_CurrentIndex++;
        D3D12_RENDER_TARGET_VIEW_DESC rtv_desc = {.Format = desc ? to_dxgi_format(desc->format) : to_dxgi_format(texture.format),
    ...
    u32 DX12DescriptorHeap::allocate_dsv(Texture& texture, const DepthStencilViewDesc* desc) {
        u32 index = m_CurrentIndex++;
        D3D12_DEPTH_STENCIL_VIEW_DESC dsv_desc = {.Format = desc ? to_dxgi_format(desc->format) : to_dxgi_format(texture.format),
                                                  .ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D,
                                                  .Flags = D3D12_DSV_FLAG_NONE,
                                                  .Texture2D = {.MipSlice = desc ? desc->mip_slice : 0u}};
        D3D12_CPU_DESCRIPTOR_HANDLE handle = get_cpu_descriptor_handle(index);
