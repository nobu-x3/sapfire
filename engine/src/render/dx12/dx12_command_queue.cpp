...
namespace sf::render::dx12 {
    DX12CommandQueue::DX12CommandQueue(ID3D12Device* device, CommandQueueType type, const wchar_t* name) : m_Type(type) {
    ...
    void DX12CommandQueue::execute_command_list(IContext* context) { execute_command_lists(&context, 1); }
    ...
    u64 DX12CommandQueue::get_last_completed_fence_value() const { return m_Fence->GetCompletedValue(); }

    bool DX12CommandQueue::is_fence_complete(u64 fence_value) const { return m_Fence->GetCompletedValue() >= fence_value; }
} // namespace sf::render::dx12
