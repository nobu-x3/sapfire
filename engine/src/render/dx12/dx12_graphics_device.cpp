...
void DX12GraphicsDevice::wait_for_idle() {
    if (m_DirectQueue) m_DirectQueue->wait_for_idle();
    if (m_ComputeQueue) m_ComputeQueue->wait_for_idle();
...
void DX12GraphicsDevice::resize_window(u32 width, u32 height) {
    ...
    dx12_check(m_Swapchain->ResizeBuffers(
        ...
        width,
        height,
        to_dxgi_format(m_BackBufferFormat),
...
Texture& DX12GraphicsDevice::get_current_back_buffer() {
    return m_BackBuffers[m_CurrentBackBufferIndex];
}

Texture& DX12GraphicsDevice::get_back_buffer(u32 index) {
...
IGraphicsContext& DX12GraphicsDevice::get_graphics_context(u32 frame_index) {
    return *m_GraphicsContexts[frame_index];
}

IComputeContext& DX12GraphicsDevice::get_compute_context() {
    return *m_ComputeContext;
}

ICopyContext& DX12GraphicsDevice::get_copy_context() {
...
void DX12GraphicsDevice::init_bindless_root_signature() {
    ...
    dx12_check(m_Device->CreateRootSignature(
        0,
        shader_result.root_signature_blob->GetBufferPointer(),
